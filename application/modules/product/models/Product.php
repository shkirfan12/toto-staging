...
//            die;
    ...
    public function fetchCompareIds($productId) {
      ...
      public function fetchTier2List($tier1Id) {
        $sql = $this->_db->select()
                ->from($this->_tier2, array('tier2_id'))
                ->where('tier1_id = ?', $tier1Id);
        try {
            $result = $this->_db->fetchAll($sql);
            return $result;
    ...
    Public function advanceSearch($param) {
        ...
        if(isset($param['tier1']) && count($param['tier1']) > 0){
            $sql = "SELECT distinct  p.product_id from product p WHERE (p.baseline = 'n') ";
            $sql .= "AND (";
              foreach ($param['tier1'] as $key3 => $value3) {
                        $tier2 = $this->fetchTier2List($value3);
                        if(count($tier2) > 0){
                            $array = array();
                            foreach ($tier2 as $key6 => $value6) {
                                $array[] = $value6['tier2_id'];                        
                            }
                            if(isset($param['tier2']) && count($param['tier2']) > 0){
                              $incResult = array_intersect($param['tier2'], $array);
                              if(count($incResult) >0){
                                    foreach ($incResult as $key4 => $value4) {                                         
                                        $sql .= "(tier1_id = $value3 AND tier2_id =  $value4) OR";
                                    }
                                }else{
                                    $sql .= "(tier1_id = $value3) OR";
                               }
                            }else{
                                $sql .= "(tier1_id = $value3) OR";
                            }
                     ...
                     if (isset($param['tag1']) && count($param['tag1'] > 0)) {
                     if(count($result) > 0){
                          foreach ($result as $keyid => $valueid) {
                            $tagID[] = $valueid['product_id'];
        }
                        $tagID = implode(',', $tagID);
                         $where = "product_id IN($tagID)";
                     }else{
                         $where = "";
        }
                        $sqlTag = "SELECT distinct  p.product_id from product p WHERE  $where ";
                         $sqlTag .= "AND (";
                        foreach ($param['tag1'] as $key => $value) {
                            $sqlTag .= "(tag1_id = $value ) OR";
        }
                        $sqlTag .= ")";
                        $sqlTag = substr_replace($sqlTag ,")",-3);

                        $result = $this->_db->fetchAll($sqlTag);
                          
                    }
                    
                    
                     if (isset($param['tag2']) && count($param['tag2'] > 0)) {
                    if(count($result) > 0){
                          foreach ($result as $keyid => $valueid) {
                            $tag2ID[] = $valueid['product_id'];
                        }
                        $tag2ID = implode(',', $tag2ID);
                         $where = "product_id IN($tag2ID)";
                     }else{
                         $where = "";
                     }
                        ...
                        $sqlTag = "SELECT distinct  p.product_id from product p WHERE  $where ";
                         $sqlTag .= "AND (";
                        foreach ($param['tag2'] as $key => $value) {
                            $sqlTag .= "(tag2_id = $value ) OR";
                        }
                        $sqlTag .= ")";
                        $sqlTag = substr_replace($sqlTag ,")",-3);
        ...
        }else if(isset($param['tier2']) && count($param['tier2']) > 0){
            $sql = "SELECT distinct  p.product_id from product p WHERE (p.baseline = 'n') ";
            $sql .= "AND (";
        ...
        }else if(isset($param['tag1']) && count($param['tag1']) > 0){
            $sql = "SELECT distinct  p.product_id from product p WHERE (p.baseline = 'n') ";
            $sql .= "AND (";
                               
                      $t1 = implode(",", $param['tag1']);
                        $sql .= "tag1_id IN ($t1)";            
        ...
        }else if(isset($param['tag2']) && count($param['tag2']) > 0){
            $sql = "SELECT distinct  p.product_id from product p WHERE (p.baseline = 'n') ";
            $sql .= "AND (";
                               
                      $t2 = implode(",", $param['tag2']);
                        $sql .= "tag2_id IN ($t2)";            
        ...
        if(isset($result) && count($result) > 0){                   
             foreach ($result as $keyid => $valueid) {
                $product_id[] = $valueid['product_id'];
              }
              $product_ids = implode(',', $product_id);
             $sql = "SELECT `p`.*, `u`.`subatt_id`, `u`.`value`, `u`.`unit` FROM `product` AS `p` LEFT JOIN `unit_value` AS `u` ON p.product_id = u.product_id WHERE (p.baseline = 'n') AND p.product_id IN ($product_ids) ORDER BY p.product_brand , p.product_id ASC"; 
        }else{
             return 'empty';            
        } 
        $result = $this->_db->fetchAll($sql);
        ...
        try {
            ...
            if(!empty($result['subatt_id'])){
               ...
               // $unit    = $result['unit'];     $unit = explode(',', $unit); 
                ...
                $sql1 = "SELECT s.*, a.* , s.rank as srank FROM sub_attributes as s
                        LEFT JOIN attributes as a
                        ON s.attribute_id=a.attribute_id
    ...
    public function getProductSubattValues($productId) {
         if(empty($productId)){ return false;}
        $sql = $this->_db->select()
                ->from($this->_uValue)
                ->where('product_id = ?', $productId);
    ...
    public function getBaselineProductUpdatedSubattValues($projectId, $productId, $baselineId) {
         if(empty($productId)){ return false;}
        $sql = $this->_db->select()
                ->from($this->_productBaseline)
                ->where('project_id = ?', $projectId)
