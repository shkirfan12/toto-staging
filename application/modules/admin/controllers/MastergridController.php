...
//        exit;
      ...
      //echo '<pre>';print_r($product);exit;
        ...
        if (count($product) > 1) {
            $importFileName =  $filename. '.xls'; 
            $mastergridModel->insertImportFileName($importFileName);
            for ($i = 2; $i < count($product); $i++) { 
               //echo "<pre>"; print_R($product); exit;
                ...
                if ($product[$i][1] != '' && $product[$i][2] != '') {
                    ...
                    if ($product[$i][4] != '' && $product[$i][4] != "0") {
                        $tier2id = $mastergridModel->gettier2idbyname($product[$i][4]);
                    } else {
                        $tier2id = '';
                    }

                    $tag1 = $mastergridModel->gettag1idbyname($product[$i][5]);
                    if ($tag1 != '') {
                        $tag1array = array();
                    ...
                    $tag2 = $mastergridModel->gettag2idbyname($product[$i][6]);
                    if ($tag2 != '') {
                        $tag2array = array();
                    ...
                    if ($product_id == 0) {
                        ...
                        if ($tier1id != '') {
                           //echo"<pre>"; print_r($product); die;
                            $trim_prod_number = str_replace(' ', '', $product[$i][1]);
                            $BL_productNum = stristr($trim_prod_number, '-', true); 
                            $BL = strtoupper($BL_productNum);
                            if($BL == 'BL') { $baseline = 'y';}else{ $baseline = 'n';} 
                           
                            $productArray = array(
                                'product_name' => $product[$i][2],
                                'product_img' => $product[$i][8],
                                'product_num' => $product[$i][1],
                                'product_brand' => $product[$i][9],
                        ...
                        if ($tier1id != '') {  
                            ...
                            $trim_prod_number = str_replace(' ', '', $p3);
                            $BL_productNum = stristr($trim_prod_number, '-', true); 
                            $BL = strtoupper($BL_productNum);
                            if($BL == 'BL') { $baseline = 'y';}else{ $baseline = 'n';} 
                    ...
                    if ($product_id != 0) {
                        ...
                        $attList = $objAtt->fetchAttListexport();
                        $subattrarrResult = array();
                        $valuesResult = array();
                        for ($a = 0; $a < count($attList); $a++) {
                            $arrResult = $objAtt->fetchSubattByAttIdexport($attList[$a]['attribute_id']); //print_R($arrResult);
                        ...
                        $countproductconf = count($subattrarrResult) + 10;
                        for ($cols = 10; $cols < $countproductconf; $cols++) {
                        ...
                        $arrcalcids = array(); 
                        if ($product[$i][$calcol] != ''){
                            $trimcalc_values= str_replace(' ','',$product[$i][$calcol]);
                            $calc_values = explode(',', $trimcalc_values);
                        ...
                        //Remove calculator records from Baseline product : put it with blank entry
                         if($BL == 'BL') {
                                 $strcalcids = '';
                         }
                        //insert >>
                        ...
                        //Vidya : Units value in decimal <<
                        foreach ($finalval as $key => $value) {
                            if (is_numeric($value)) {
                                $t = sprintf('%0.2f', $value);
                                $finalval[$key] = $t;
                            }else{
                               $finalval[$key] = $value;
                            } 
       ...
       // $this->_redirect('/admin/mastergrid');
        $this->_redirect('/admin/import-management/');
    }
...
// Set properties
        ...
        $objPHPExcel->getActiveSheet(0)->mergeCells("B1:J1");
        ...
        $planetcolor = array(
         ...
         $peoplecolor = array(
            ...
            'fill' => array(
                'type' => PHPExcel_Style_Fill::FILL_SOLID,
                'color' => array('rgb' => 'c51e49')
            )
        );
          $fetchTiersTagsNameList = $objAtt->fetchTiersTagsNameList();
        // Sets array for each attbutes and its respective sub attributes
        $colorbg = array();
        $arrAttSub = array();  
        $columnnumber = '10';
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValueByColumnAndRow('3', '2', $fetchTiersTagsNameList['tier1_name']);

        $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValueByColumnAndRow('4', '2', $fetchTiersTagsNameList['tier2_name']);

        $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValueByColumnAndRow('5', '2', $fetchTiersTagsNameList['tag1_name']);

        $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValueByColumnAndRow('6', '2', $fetchTiersTagsNameList['tag2_name']);
        $objPHPExcel->setActiveSheetIndex(0)
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValueByColumnAndRow('9', '2', 'Product Brand');
...
//                ->setCellValueByColumnAndRow('10', '2', 'Technology');
        for ($a = 0; $a < count($attList); $a++) {
            ...
            if (preg_match('/3/', $attList[$a]['attribute_id'])) {
                $colorbg = $watercolor;
            } else if (preg_match('/2/', $attList[$a]['attribute_id'])) {
                $colorbg = $energycolor;
            } else if (preg_match('/4/', $attList[$a]['attribute_id'])) {
                $colorbg = $lifecyclecolor;
            } else if (preg_match('/5/', $attList[$a]['attribute_id'])) {
                $colorbg = $humancolor;
            } else if (preg_match('/7/', $attList[$a]['attribute_id'])) {
                $colorbg = $planetcolor;
            }else if (preg_match('/People/', $attList[$a]['attribute_id'])) {
                $colorbg = $peoplecolor;
            }
        ...
        foreach ($prodData as $key1 => $val1) { 
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                   // ->setCellValueExplicit('E' . $num1, $val1['product_name']);
                    ->setCellValueExplicit('E' . $num1, $tier2);
...
//            }
          //    print_r($tag2);
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValueExplicit('F' . $num1, $tag1[0]['tag1_name']);

          
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValueExplicit('G' . $num1, $tag2[0]['tag2_name']);
            $objPHPExcel->setActiveSheetIndex(0)
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValueExplicit('J' . $num1, $val1['product_brand']);
//            $objPHPExcel->setActiveSheetIndex(0)
//                    ->setCellValueExplicit('K' . $num1, '');
            $columnnumber = '10';
            $endcolumnnumber = '';
            $subattcolumnnumber = '10';
            // $val1['product_name'] = 'asaa&,\"';
           // echo "<pre>"; print_r($attList); 
            for ($a = 0; $a < count($attList); $a++) {
                $arrResult = $objAtt->fetchSubattByAttIdexport($attList[$a]['attribute_id']); 
                if(count($arrResult) > 0){
            ...
            foreach ($calculatorname as $key1 => $val2) {
                array_push($calculatornamearray, $val2['cal_code']);
            }     
           
            $calculatornamearray = implode(',', $calculatornamearray);
            
            //Remove calculator records from Baseline product : put it with blank entry
            $trim_prod_number = str_replace(' ', '', $val1['product_num']);
            $BL_productNum = stristr($trim_prod_number, '-', true); 
            $BL = strtoupper($BL_productNum);
            if($BL == 'BL') { $calculatornamearray = "";} 
            $objPHPExcel->setActiveSheetIndex(0)
...
// Redirect output to a clientÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¢s web browser (Excel5)
        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment;filename="'.APP_TITLE .'_Database_'.$date.'_'.$time.'.xls"');
    ...
    public function exportBlankTemplateAction(){
           error_reporting(0);
        $this->_helper->layout->disableLayout();
        $this->_helper->viewRenderer->setNoRender();
        require_once $_SERVER['DOCUMENT_ROOT'] . '/common/Classes/PHPExcel.php';
...
// Set properties
        ...
        $fetchTiersTagsNameList = $objAtt->fetchTiersTagsNameList();
//        print_r($fetchTiersTagsNameList);
//        die();
        // Sets array for each attbutes and its respective sub attributes
        $colorbg = array();
        $arrAttSub = array();  
...
// Redirect output to a clientÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¢s web browser (Excel5)
    ...
    //}
         ...
         $time = date("h-i-s", time());
      header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment;filename="'.APP_TITLE .'_Blank Database_'.$date.'_'.$time.'.xls"');
        header('Cache-Control: max-age=0');
